...
class MappingsTests(test.TestCase):
  ...
  "two", 
  "three"
]</pre>''', mappings.stack_output(['one', 'two', 'three']))
        self.assertEqual(u'''<pre>{
  "foo": "bar"
}</pre>''', mappings.stack_output({'foo': 'bar'}))
...
class LaunchStackTests(test.TestCase):
    ...
    def test_launch_stack_workflow(self):
        # TODO sbaker enable and implement this test when it is find out why
        # self.client.get(url) returns HttpResponseRedirect to the login page
        self.mox.ReplayAll()
