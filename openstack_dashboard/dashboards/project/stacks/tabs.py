...
class StackEventsTab(tabs.Tab):
    ...
    def get_context_data(self, request):
        ...
        try:
            events = api.heat.events_list(self.request, stack.stack_name)
...
class StackResourcesTab(tabs.Tab):
    ...
    def get_context_data(self, request):
        stack = self.tab_group.kwargs['stack']
        try:
            resources = api.heat.resources_list(self.request, stack.stack_name)
        ...
        return {"stack": stack,
                "table": ResourcesTable(request, data=resources), }
